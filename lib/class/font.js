...
var _generateGlyph = function(value) {
  ...
  if (_.isObject(value)) {
    //需要解析好d参数，对于svg直接转换
    return new Glyph(_.extend({},value))
  }
...
var Font = Base.extend({
  ...
  init: function(options) {
    ...
    this.__viewbox = new Base({
      ...
      descent:this.__fontface.get('descent'),
      ascent:this.__fontface.get('ascent'),
    })
  },
  /**
   * 获取当前字体的fontface
  ...
  setFontface: function(options) {
    ...
    this.__viewbox.set('descent',this.__fontface.get('descent'))
    this.__viewbox.set('ascent',this.__fontface.get('ascent'))
  },
  ...
  setGlyph: function() {
    ...
    _.each(map, function(value, unicode) {
      ...
      var glyph = _generateGlyph(value)
      ...
      //设置对应的新字体，进行各种适配转换
      glyph.setViewbox(self.__viewbox)
